import { MOCK_USER_ID } from '../../user/model/mocks';
import { creatorFixDate } from '../model/creators';
import { FixDate } from '../model/types';



/** 2023-03-13 */
export const MOCK_DATE_13_03_2023 = 1678665600000;

export const MOCK_FIX_DATE: FixDate = creatorFixDate(MOCK_USER_ID, MOCK_DATE_13_03_2023);

export const MOCK_EMAIL     = 'korzan.va@mail.ru';
export const MOCK_PASSWORD  = 'R1q@678doP';

export const MOCK_ID_1      = 'jG0U0iVFdrblBRoFCsD1';
export const MOCK_ID_2      = 'jG0U0iVFdrblBRoFCsD2';
export const MOCK_ID_3      = 'jG0U0iVFdrblBRoFCsD3';
export const MOCK_PARENT_ID = 'TyfJA9mnTwUcOYVOXnQl';
