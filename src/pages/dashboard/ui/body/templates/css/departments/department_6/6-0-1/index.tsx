...
/** Кол-во новых клиентов, переданных персональному менеджеру */
export const DashboardReportContainer6_0_1 = memo(() => {
  const { activeEntities, activeDates } = useDashboardData();

  const itemData  = useMemo(() => activeEntities['6-0-1'] as DashboardStatisticItem<number>, [activeEntities]);
  const condition = useMemo(() => getConditionType(activeEntities['6-0-1-C']?.data), [activeEntities]);
